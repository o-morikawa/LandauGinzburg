 ...
 */
const double MAX_NRERR     = 1.0e-13;
/**
 ...
 *        Parallel computing with openmp (for Nicolai maps)
 * \see lg_params_t
 */
...
/**
 * \fn    void NicolaiConf_ord (const lg_params_t params)
 * \brief Solve a Landau--Ginzburg model by NR method;
 *        Parallel computing with openmp (for Solutions of one Nicolai)
 * \see lg_params_t
 */
void NicolaiConf_ord (const lg_params_t lp) {
  try {
    if (lp.Li % 2 != 0)
      throw "ERROR<NicolaiConf_ord> Box size is not EVEN.";

    if      (lp.spt==SuperPotentialType::AlgebraA) {
	  if (lp.num_f         != 1)
		throw "ERROR<NicolaiConf_ord> SuperPotentialType::AlgebraA: number of fields should be 1.";
	  if (lp.k.size()      != 1)
		throw "ERROR<NicolaiConf_ord> SuperPotentialType::AlgebraA: number of powers should be 1.";
	  if (lp.lambda.size() != 1)
		throw "ERROR<NicolaiConf_ord> SuperPotentialType::AlgebraA: number of couplings should be 1.";
	}
    else if (lp.spt==SuperPotentialType::AlgebraD) {
	  if (lp.num_f         != 2)
		throw "ERROR<NicolaiConf_ord> SuperPotentialType::AlgebraD: number of fields should be 2.";
	  if (lp.k.size()      != 1)
		throw "ERROR<NicolaiConf_ord> SuperPotentialType::AlgebraD: number of powers should be 1.";
	  if (lp.lambda.size() != 2)
		throw "ERROR<NicolaiConf_ord> SuperPotentialType::AlgebraD: number of couplings should be 2.";
	}
	else if (lp.spt==SuperPotentialType::AlgebraE) {
	  if (lp.num_f         != 2)
		throw "ERROR<NicolaiConf_ord> SuperPotentialType::AlgebraE: number of fields should be 2.";
	  if (lp.lambda.size() != 2)
		throw "ERROR<NicolaiConf_ord> SuperPotentialType::AlgebraE: number of couplings should be 2.";
	}
	else {
	  if (lp.num_f     < 1)
		throw "ERROR<NicolaiConf_ord> Number of fields should be positive.";
	  if (lp.k.size() == 0)
		throw "ERROR<NicolaiConf_ord> No values in powers.";
	  if (lp.lambda.size() == 0)
		throw "ERROR<NicolaiConf_ord> No values in couplings";
	}

    cerr << endl << "Calc start ..." << endl;
    for (int in = lp.num_initialnic; in < lp.num_initialnic + lp.num_nicolai; ++in) {
      NicolaiSol nic(lp.Li, lp.num_f, in);
      double dmp = nic.nr_method_omp(lp.k, lp.lambda, lp.num_nrsolutions, lp.num_loop, lp.spt);
